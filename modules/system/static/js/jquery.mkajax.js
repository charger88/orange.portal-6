/*
MKAjax v1.0.2
Copyright (c) 2013, 2014, 2015 Mikhail Kelner
    ...
    jQuery.fn.mkajax = function(options){
        options = $.extend({
			...
			onRequestError: function(destination,response){
                if (options.debug){
                    console.log('onRequestError',destination,response);
                }
			...
			stopLoadingAnimation: function(object){
				...
				$plugin.on('submit',function(){
					...
					$.ajax({
						...
						'error': function(response){
							...
							options.stopLoadingAnimation($form);
				...
				$plugin.on('click',function(){
					...
					$.ajax({
						...
						'error': function(response){
							...
							options.stopLoadingAnimation($a);
